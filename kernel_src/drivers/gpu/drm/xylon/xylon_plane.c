			   ...
			   u32 src_w, u32 src_h)
{
	...
	struct xylon_drm_plane_manager *manager = plane->manager;
	int id = plane->id;
        int pixel_format = base_plane->format_types[0];
	int ret;

	const struct drm_format_info *info;
	info = __drm_format_info(pixel_format);
	
	if (fb->format->format != info->format) {
		DRM_ERROR("unsupported pixel format %08x %08x\n",
			fb->format->format, info->format);
		return -EINVAL;
	}
	...
	DRM_DEBUG("paddr: %p, h: %d(%d), v: %d(%d), bpp: %s\n",
		       ...
		       unsigned int primary_id)
{
	...
	struct xylon_cvc *cvc = manager->cvc;
        const uint64_t *format_modifier = 0;
	...
	ret = drm_universal_plane_init(manager->dev, &plane->base,
				       ...
				       &format, 1, format_modifier, type, NULL);
	if (ret) {
